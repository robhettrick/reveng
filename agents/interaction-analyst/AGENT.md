...
- **Append with `cat >>`, not Edit.** Create the file with the **Write** tool (metadata block plus the first section), then append every subsequent section with a single heredoc. Use Write for creation rather than `cat >`, so only appends go through Bash:
  ...
  Write the path exactly as shown — relative, `output/interaction-analysis.md` — not an absolute path. The CLI always runs from the workspace root, so the relative form is correct in every environment; an absolute path differs between a container run and a host run and will not match the permitted command.
