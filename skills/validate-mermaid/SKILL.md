...
4. **Validate each block.**
   ...
   **Using `mmdc`:** use the **Write** tool to put the block's content (excluding
   the fence lines) into a scratch file **under `/tmp`**, then render it:

   ```
   mmdc -i /tmp/mermaid-check-N.mmd -o /tmp/mermaid-check-N.svg
   ```

   **Never write these files into the workspace.** They are throwaway validation
   scratch, not deliverables: writing them next to the markdown file leaves
   `.mmd` and `.svg` clutter in `output/` beside the analyses, where a later
   agent may mistake them for content and a reader has to work out what they are.
   `/tmp` is outside the workspace, so nothing needs cleaning up afterwards.
   ...
   Use a distinct `N` per block so concurrent agents do not collide.
