...
allowed-tools: Read, Grep, Glob, Edit(.claude-review/findings.json), Bash(git diff:*), Bash(git log:*), Bash(git show:*), Bash(git merge-base:*), Bash(npm test:*), Bash(npm run lint:*), Bash(npm --prefix:*), Bash(python3 .github/claude/filter_findings.py:*)
---

Review the current branch against `$ARGUMENTS` (default `main`) using the brief in `.github/claude/pr-review-prompt.md`. Substitute `{{RUN_CONTEXT}}` with the repo, base ref, HEAD sha and the test/lint commands for each directory that has a `package.json` (`npm --prefix <dir> test`, `npm --prefix <dir> run lint`); do not install anything.
